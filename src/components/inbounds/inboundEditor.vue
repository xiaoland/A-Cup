...
import { InboundSchema, type Inbound } from "../../../schemas/inbound";
...
const onTypeChange = (newType: "mixed" | "tun") => {
    ...
    try {
        ...
        console.error("Failed to parse inbound configuration:", error);
    }
};
...
const tunAddress = computed({
    get: () => {
        if (inbound.value.type === "tun") {
            if (Array.isArray(inbound.value.address))
                return inbound.value.address;
            if (typeof inbound.value.address === "string")
                return [inbound.value.address];
        }
        return [];
    },
    ...
    <div class="flex flex-col gap-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <InputText id="tag" v-model="inbound.tag" class="w-full" />
            </div>
        ...
        <template v-if="inbound.type === 'mixed'">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="listen" class="block mb-2 font-medium"
                ...
                <div>
                    <label for="listen_port" class="block mb-2 font-medium"
                        >Listen Port</label
                    >
                    <InputNumber
                        id="listen_port"
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <small class="text-gray-500"
                            >Press Enter to add multiple addresses</small
                        >
                    </div>

                    <div>
                        <label for="mtu" class="block mb-2 font-medium"
                            >MTU</label
                        >
                        <InputNumber
                            id="mtu"
                    ...
                    <div>
                        <label for="stack" class="block mb-2 font-medium"
                            >Network Stack</label
                        >
                        <Select
                            id="stack"
