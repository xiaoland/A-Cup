...
watch(
    ...
    (newType, oldType) => {
        ...
        // Use schema.parse() with empty object to get default values for credentials
        switch (newType) {
            case "vless":
...
function save() {
    // Validate before saving using schema
    try {
        ...
        <Fieldset legend="Basic Info">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <InputText
                        id="provider"
                        v-model="localOutbound.provider"
                        class="w-full"
                    />
                </div>
            </div>
        ...
        <Fieldset legend="Base" class="mt-4">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <InputNumber
                        id="server_port"
                        v-model="localOutbound.server_port"
                        class="w-full"
                    />
                </div>
            </div>
            ...
            <div class="mt-4">
                <VlessForm
                    v-if="localOutbound.type === 'vless'"
                    v-model="localOutbound.credential"
                />
                <VmessForm
                    v-if="localOutbound.type === 'vmess'"
                    v-model="localOutbound.credential"
                />
                <ShadowsocksForm
                    v-if="localOutbound.type === 'shadowsocks'"
                    v-model="localOutbound.credential"
                />
                <Hysteria2Form
                    v-if="localOutbound.type === 'hysteria2'"
                    v-model="localOutbound.credential"
                />
            </div>
        </Fieldset>
