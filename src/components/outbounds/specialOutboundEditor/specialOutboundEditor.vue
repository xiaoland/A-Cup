...
import {
    ...
    SingBoxOutboundSchema,
    type SingBoxOutbound,
} from "../../../../schemas/singbox";
...
type SpecialOutboundModel =
    | z.infer<typeof SelectorOutboundSchema>
    | z.infer<typeof UrlTestOutboundSchema>
    | z.infer<typeof DirectOutboundSchema>;
...
onMounted(() => {
    if (!localOutbound.value) {
        // Use schema.parse() to get proper defaults
...
watch(
    () => localOutbound.value?.type,
    (newType, oldType) => {
        if (!newType || newType === oldType) return;

        const currentTag = localOutbound.value?.tag || "new-special-outbound";

        // Use schema.parse() to get proper defaults for each type
        try {
            switch (newType) {
                case "selector":
                    localOutbound.value = SelectorOutboundSchema.parse({
                ...
                case "urltest":
                    ...
                    break;
                case "direct":
                    localOutbound.value = DirectOutboundSchema.parse({
                        tag: currentTag,
                    });
                    break;
            }
            ...
            console.error(
                "Failed to parse special outbound configuration:",
                error,
            );
        }
    },
    ...
    <div v-if="localOutbound" class="flex flex-col gap-4">
        ...
        <Fieldset legend="Configuration" class="w-full">
            <div class="flex flex-col gap-4">
                <SelectorForm
                    v-if="localOutbound.type === 'selector'"
                    v-model="localOutbound"
                />
                <UrltestForm
                    v-if="localOutbound.type === 'urltest'"
                    v-model="localOutbound"
                />
                <DirectForm
                    v-if="localOutbound.type === 'direct'"
                    v-model="localOutbound"
                />
            </div>
        </Fieldset>
