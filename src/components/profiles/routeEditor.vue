...
const final = computed({
    ...
    set: (value) =>
        emit("update:modelValue", { ...props.modelValue, final: value }),
});
...
const advancedFields = computed(() => {
    const { rules, rule_set, final, auto_detect_interface, ...rest } =
        props.modelValue;
    return rest;
});
...
const getRuleSetName = (ruleSet: SingBoxRuleSet): string => {
    // Try to parse the tag as an ID and look up the name in the store
    const ruleSetId = parseInt(ruleSet.tag);
    if (!isNaN(ruleSetId)) {
        const storedRuleSet = ruleSetStore.ruleSets.find(
            (rs) => rs.id === ruleSetId,
        );
        if (storedRuleSet) {
            return storedRuleSet.name;
        }
    }
    // Fallback to the tag if not found in store
    return ruleSet.tag || "Unnamed Rule Set";
};
</script>
    ...
    <div class="p-4">
        ...
        <div class="border border-gray-300 rounded-lg p-4 mb-4 bg-gray-50">
            ...
            <div class="grid gap-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="final" class="block mb-2 font-medium"
                            >Final Outbound</label
                        >
                        <SingBoxOutboundPicker id="final" v-model="final" />
                        <small class="block mt-1 text-gray-500"
                            >Outbound to use when no rule matches</small
                        >
                    </div>
        ...
        <Fieldset legend="Route Rules" class="mt-4">
            ...
            <div v-else class="text-center py-6 text-gray-500">
                No route rules configured. Add one to route traffic based on
                conditions.
            </div>
            <Button
        ...
        <Fieldset legend="Rule Sets" class="mt-4">
            <div v-if="route.rule_set && route.rule_set.length > 0">
                <Accordion :multiple="true" class="mb-3">
                    <AccordionPanel
                        v-for="(ruleSet, index) in route.rule_set"
                        :key="index"
                        :value="`ruleset-${index}`"
                    >
                        <AccordionHeader>
            ...
            <div v-else class="text-center py-6 text-gray-500">
                No rule sets configured. Add one to define reusable routing
                rules.
            </div>
            <Button
                label="Add Rule Set"
        ...
        >
            <JSONEditor
                :modelValue="advancedFields"
                @update:modelValue="onAdvancedChange"
            />
