  ...
  <v-card variant="outlined" class="form-section">
    ...
    <v-card-text>
      ...
      <div class="mb-6 d-flex flex-column gap-4">
        <div v-for="(server, sidx) in dns.servers" :key="`server-${sidx}`">
      ...
      <div class="d-flex flex-column gap-4">
        <div v-for="(rule, ridx) in dns.rules" :key="`rule-${ridx}`">
          <div class="d-flex align-center justify-space-between mb-2">
      ...
      <v-expansion-panels class="mt-2">
        <v-expansion-panel>
          ...
          <v-expansion-panel-text>
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field v-model="dns.final" label="Final Server Tag" variant="outlined" hint="Default DNS server tag; first server used if empty" persistent-hint />
              </v-col>
              <v-col cols="12" md="6">
                <v-select
            ...
            <v-row>
              <v-col cols="12" md="4">
                <v-switch v-model="dns.disable_cache" label="Disable Cache" color="primary" />
              </v-col>
              <v-col cols="12" md="4">
                <v-switch v-model="dns.disable_expire" label="Disable Expire" color="primary" />
              </v-col>
              <v-col cols="12" md="4">
                <v-switch v-model="dns.independent_cache" label="Independent Cache" color="primary" />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12" md="4">
                <v-text-field v-model.number="dns.cache_capacity" type="number" label="Cache Capacity" variant="outlined" hint="Value < 1024 ignored" persistent-hint />
              </v-col>
              <v-col cols="12" md="4">
                <v-switch v-model="dns.reverse_mapping" label="Reverse Mapping" color="primary" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="dns.client_subnet" label="Client Subnet" variant="outlined" hint="edns0-subnet; CIDR or IP" persistent-hint />
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col cols="12" md="4">
                <v-switch v-model="dns.fakeip!.enabled" label="Enable FakeIP" color="primary" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="dns.fakeip!.inet4_range" label="IPv4 Range" placeholder="198.18.0.0/15" variant="outlined" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="dns.fakeip!.inet6_range" label="IPv6 Range" placeholder="fc00::/18" variant="outlined" />
              </v-col>
            </v-row>
          </v-expansion-panel-text>
        </v-expansion-panel>
