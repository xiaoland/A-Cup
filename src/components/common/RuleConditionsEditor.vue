...
const addCondition = () => {
    if (selectedConditionToAdd.value) {
        const conditionType = conditionTypes.find(
            (c) => c.value === selectedConditionToAdd.value,
        );
        if (conditionType) {
...
<template>
    <div class="rule-conditions-editor">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
                v-for="key in visibleConditions"
                :key="key"
        ...
        <div
            v-else-if="visibleConditions.length === 0"
            class="text-center py-4 text-500"
        >
            <p>No conditions added yet.</p>
            <p class="text-sm mt-2">
                Use the "Add Condition" button below to add conditions.
            </p>
        </div>
    </div>
</template>
