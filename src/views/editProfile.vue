...
onMounted(async () => {
    ...
    if (profileId) {
        const draftKey = profileStore.getDraftKeyForEdit(profileId);
        const draft = profileStore.loadDraft(draftKey);
        if (draft) {
            profile.value = draft;
        } else {
            try {
        ...
        if (profile.value) {
            ...
            if (profileId) {
                profileStore.saveDraft(
                    profileStore.getDraftKeyForEdit(profileId),
                    profile.value,
                );
            }
        }
...
const handleSave = async (updatedProfile: CreateProfile) => {
    ...
    if (profileId) {
        try {
            ...
            profileStore.clearDraft(profileStore.getDraftKeyForEdit(profileId));
