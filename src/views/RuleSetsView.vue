<template>
  <div class="rule-set-list">
    <Card>
      ...
      <template #content>
        <DataTable
          ...
          v-model:selection="selectedRuleSets"
        >
    ...
    <Dialog v-model:visible="bulkDeleteDialog.show" modal header="Bulk Delete Rule Sets" class="w-full max-w-md">
       <div class="p-4">
            Are you sure you want to delete <strong>{{ selectedRuleSets.length }}</strong> selected rule sets? This action cannot be undone.
       </div>
      <template #footer>
        <Button
          label="Cancel"
        ...
        <Button
          label="Delete"
          severity="danger"
          @click="bulkDeleteRuleSets"
          :loading="bulkDeleteDialog.deleting"
        />
      </template>
    </Dialog>
  </div>
...
import { exportTypes } from '@/types/route'
...
const deleteRuleSet = async () => {
  if (!deleteDialog.value.ruleSet || !deleteDialog.value.ruleSet.id) return
...
const bulkDeleteRuleSets = async () => {
  bulkDeleteDialog.value.deleting = true
  const idsToDelete = selectedRuleSets.value.map((rs: RuleSet) => rs.id).filter(id => id !== undefined) as number[]
  await Promise.all(idsToDelete.map((id: number) => ruleSetStore.deleteRuleSet(id)))
  selectedRuleSets.value = []
  bulkDeleteDialog.value.deleting = false
...
const toggleMenu = (ruleSet: RuleSet, event: any) => {
  menu.value.model = getMenuItems(ruleSet)
