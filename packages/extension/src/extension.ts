...
export function activate(context: vscode.ExtensionContext) {
	...
	const uploadToOSSCommand = vscode.commands.registerCommand(
		'a-cup.uploadToOSS',
		async () => {
			...
			if (!endpoint || !accessKeyId || !accessKeySecret || !bucket) {
				vscode.window.showErrorMessage(
					'Please configure OSS settings in extension settings',
				);
				return;
			}
			...
			const fileName =
				editor.document.fileName.split('/').pop() || 'profile.json';

			// TODO: Implement actual OSS upload logic
			vscode.window.showInformationMessage(
				`Upload to OSS: ${fileName} (${content.length} bytes)`,
			);
		},
	);

	// Register download command
	const downloadFromOSSCommand = vscode.commands.registerCommand(
		'a-cup.downloadFromOSS',
		async () => {
			const config = vscode.workspace.getConfiguration('a-cup.oss');
			const endpoint = config.get<string>('endpoint');
			const accessKeyId = config.get<string>('accessKeyId');
